...
from smarttvleakage.audio.sounds import KEY_SELECT_SOUNDS, CHANGE_SOUNDS, SPACE_SOUNDS, DONE_SOUNDS
...
def build_node(clf, node):
    ...
    dict["class"] = clf.tree_.value[node]
...
def update_dict(data, base):
    if data["feature"] == -2:
        return base


    if data["feature"] not in base:
        base[data["feature"]] = 1
    else:
        base[data["feature"]] += 1

    if data["left"] != -1:
        base = update_dict(data["left"], base)
    if data["right"] != -1:
        base = update_dict(data["right"], base)
    return base

def update_dict_weighted(data, base, weight):
    if data["feature"] == -2:
        return base


    if data["feature"] not in base:
        base[data["feature"]] = weight
    else:
        base[data["feature"]] += weight

    if data["left"] != -1:
        base = update_dict_weighted(data["left"], base, weight/2)
    if data["right"] != -1:
        base = update_dict_weighted(data["right"], base, weight/2)
    return base
        
    


def get_feature_data(forest, weight = -1):
    base = {}
    for key, val in forest.items():
        if weight == -1:
            base = update_dict(val, base)
        else:
            base = update_dict_weighted(val, base, weight)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, required=False) #type of classifier
    ...
    if mode == 0:
        ...
        if args.type is None:
            ...
            if args.save_path is None:
                ...
                feature_data = get_feature_data(forest, -1)
                choices = 0
                feature_data_sorted = sorted(feature_data.items(), reverse=True, key=lambda x:x[1])
                for feature, uses in feature_data_sorted:
                   print("feature: " + str(feature) + ", used: " + str(uses))
                   choices += uses
                print("total choices: " + str(choices))
            ...
            for i, clf in enumerate(model.estimators_):
                models[i] = build_node(clf, 0)
                models[i]["weight"] = model.estimator_weights_[i]
                models[i]["error"] = model.estimator_errors_[i]
            ...
            if args.save_path is None:
                ...
                feature_data = get_feature_data(models)

                choices = 0
                for feature, uses in feature_data.items():
                   print("feature: " + str(feature) + ", used: " + str(uses))
                   choices += uses
                print("total choices: " + str(choices))
