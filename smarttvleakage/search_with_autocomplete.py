...
def get_words_from_moves(num_moves: List[int], graph: MultiKeyboardGraph, dictionary: CharacterDictionary, max_num_results: Optional[int]) -> Iterable[Tuple[str, float, int]]:
    ...
    single_autocomplete = read_json('graphs/autocomplete.json')
    ...
    while not candidate_queue.empty():
        ...
        print('Current String: {}, Neighbors: {}, Current Key: {}, Center Key: {}'.format(current_string, neighbors, current_state.current_key, current_state.center_key))
        ...
        if (prev_key in string.ascii_letters) and ((num_moves[move_idx] == 1) or (num_moves[move_idx] <= 2 and current_state.was_on_autocomplete)) and (len(current_string) > 0):
            ...
            if len(current_string) == 1:
                ...
                next_key_counts = dictionary.get_letter_counts(prefix=current_string, should_smooth=False)
                total_count = sum(next_key_counts.values())
                next_key_freq = { key: count / total_count for key, count in next_key_counts.items() }

                sorted_keys = list(reversed(sorted(next_key_freq.items(), key=lambda t: t[1])))
                sorted_keys = list(filter(lambda t: (t[0] in string.ascii_letters) or (t[0] == '\''), sorted_keys))

            autocomplete_base_keys = graph.get_keys_for_moves_from(start_key=current_state.center_key,
                                                                   num_moves=1,
                                                                   mode=current_state.keyboard_mode)

            for (neighbor_key, score) in sorted_keys:
                if score < 0.005:
                    continue
                ...
                if candidate_word not in visited:
                    ...
                    for base_key in autocomplete_base_keys:
                        next_state = SearchState(keys=candidate_keys,
                                                 score=score * current_state.score,
