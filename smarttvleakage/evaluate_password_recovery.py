...
def process_record(record: Dict[str, Any], graph: MultiKeyboardGraph, dictionary_path: str, tv_type: SmartTVType, max_num_results: int, precomputed_path: Optional[str]) -> Tuple[int, List[str]]:
...
if __name__ == '__main__':
    ...
    for record_chunk in chunked(read_jsonl_gz(args.benchmark_path), CHUNK_SIZE):
        ...
        process_fn = partial(process_record, graph=graph, dictionary_path=args.dictionary_path, precomputed_path=args.precomputed_path, tv_type=tv_type, max_num_results=args.max_num_results)
        with Pool(4) as pool:
